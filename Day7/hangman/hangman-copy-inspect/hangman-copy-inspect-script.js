//== rewrite me to simpler code (like you did in todolist-copy-inspect.html)
...
let alreadyGuessed = [];
function checkWin() {
  wordGuessContainer.innerHTML.split(" ").join("") === chosenWord &&
    (alert(`You won! You guessed ${chosenWord} correctly!`), startNewGame());
}
function checkValidGuess(e) {
  return e.match(alphabet) && 1 === e.length
    ? alreadyGuessed.includes(e)
      ? (alert(`You have already used the letter ${e}, pick another!`), !1)
      : (alreadyGuessed.push(e), !0)
    : (alert("Invalid guess! Pick a letter a-z!"), !1);
}
function updateGuessList(e) {
  let n = document.createElement("li");
  (n.innerHTML = e), guessedLettersContainer.append(n);
}
function findInWord(e) {
  ...
  for (i = 0; i < n.length; i++)
    if (e == n[i]) {
      ...
      (n[i] = e), (wordGuessContainer.innerHTML = n.join(" ")), (s = !0);
    }
  return s;
}
guessLetterForm.onsubmit = function(e) {
  guessesRemaining > 0 &&
    ...
    checkValidGuess(currentGuess) &&
      (updateGuessList(currentGuess),
      findInWord(currentGuess)
        ? checkWin()
        : (guessesRemaining--,
          (userGuessesLeft.innerHTML = guessesRemaining),
          0 === guessesRemaining &&
